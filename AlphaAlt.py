...
class encoder_block(nn.Module):
    def __init__(self, in_dim, out_dim):
        ...
        self.net = nn.Sequential(
            nn.Linear(in_dim,in_dim),  # First linear layer
            nn.GELU(),  # ReLU activation
            nn.Linear(in_dim, in_dim),  # First linear layer
            nn.GELU(),  # ReLU activation
            nn.Linear(in_dim, out_dim))  # Second linear layer
...
class AdaptAlt(nn.Module):
    ...
    def forward(self, obsrv,mask, eps_x,eps_z,obsr_enable):
        ...
        self.alpha[0] = torch.sigmoid(self.alpha_encoder(vs))*(1 - torch.pow(self.sigma_z, 2)-1e-4)
        ...
        for k in range(1, seq_len):
            ...
            if k < self.vendi_L:
                vs=score(self.z_hat[:k],squared_error,q=self.vendi_q)*torch.ones( batch_size, 1).to(self.device)
            else:
                vs=score(self.z_hat[k-self.vendi_L:k],squared_error,q=self.vendi_q)*torch.ones( batch_size, 1).to(self.device)
            self.alpha[k] = torch.sigmoid(self.alpha_encoder(vs)) * (1 - torch.pow(self.sigma_z, 2) - 1e-4)
            if obsr_enable:
                if mask[k]:
                    ...
                    self.z_hat[k] = torch.sqrt(self.alpha[k]) * self.f_phi_x(
                        self.x_hat[k ],k ) + torch.sqrt(1 - self.alpha[k] - torch.pow(self.sigma_z, 2)) * \
                                      self.z_hat[
                                          k - 1].clone() + eps_z
                    ...
                    self.z_hat[k] =torch.sqrt(self.alpha[k]) * self.f_phi_x(
                            self.obsrv[k],k)+ torch.sqrt(1-self.alpha[k]- torch.pow(self.sigma_z,2)) *self.z_hat[k-1].clone() + eps_z
                ...
                self.z_hat[k] = torch.sqrt(self.alpha[k]) * self.f_phi_x(
                    self.x_hat[k], k) + torch.sqrt(1 - self.alpha[k] - torch.pow(self.sigma_z, 2)) * self.z_hat[
                                      k - 1].clone() + eps_z
    ...
    def loss (self, a,b,c,z):
        ...
        L2 = F.mse_loss( z[1:], torch.sqrt(1 - self.alpha[:-1] - torch.pow(self.sigma_z, 2)) * self.z_hat[:-1]+
                        torch.sqrt(self.alpha[1:]) * self.z_x_hat[1:])*self.sigma_x**2
